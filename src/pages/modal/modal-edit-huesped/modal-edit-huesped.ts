...
@IonicPage()
@Component({
  selector: 'page-modal-edit-huesped',
  templateUrl: 'modal-edit-huesped.html'
})
export class ModalEditHuesped {
  ...
  public id_img: any;
  public subir_imagenes: boolean = false;
  ...
  public server = "https://dashboard.becheckin.com/imgs/";
  ...
  constructor(
    ...
    // conseguimos el huesped que estamos editando:
    ...
    this.storageService.getUserData().then((userData) => {
      ...
      // this.user.guest = this.huesped;
      // this.user.guest = new Guest();
      this.guestCopy = this.globalService.clone(this.huesped);
      this.actualGuestCopy = this.globalService.clone(this.serviceAPI.actualGuests);
      this.user.guest = this.huesped;
  ...
  public cancelar() {
    this.huesped = this.globalService.clone(this.guestCopy);
    this.user.guest = this.huesped;
    this.storageService.setUserData(this.user);
    this.serviceAPI.actualGuests = this.actualGuestCopy;
    console.log("servicio huespedes: ", this.serviceAPI.actualGuests);
    this.viewCtrl.dismiss();
  }

  public vuelcaPhotosRecuperadas() {

    // Comprobamos si es un usuario con imágenes, en caso contrario lo damos como una inserción:
    if (this.huesped.fastcheckin.imagenes == undefined) {
      this.paso = 1;
      this.vuelcaPhotos();
    }
    else {
      //Para el DNI:
      if (this.huesped.fastcheckin.typeOfDocument == 'D') {

        if (this.huesped.fastcheckin.imagenes[0] != undefined)
          this.photosNif.push(this.server + this.huesped.fastcheckin.imagenes[0].enlace + "/" + this.huesped.fastcheckin.imagenes[0].nombre);
        if (this.huesped.fastcheckin.imagenes[1] != undefined)
          this.photosNif.push(this.server + this.huesped.fastcheckin.imagenes[1].enlace + "/" + this.huesped.fastcheckin.imagenes[1].nombre);
      } else if (this.huesped.fastcheckin.typeOfDocument == 'E') {
        if (this.huesped.fastcheckin.imagenes[0] != undefined)
          this.photosEmpleado.push(this.server + this.huesped.fastcheckin.imagenes[0].enlace + "/" + this.huesped.fastcheckin.imagenes[0].nombre);
      } else {
        //Para pasaporte:
        if (this.huesped.fastcheckin.imagenes[0] != undefined)
          this.photosPassport.push(this.server + this.huesped.fastcheckin.imagenes[0].enlace + "/" + this.huesped.fastcheckin.imagenes[0].nombre);
      }
    }
  ...
  saveProfilemio(loading) {
    ...
    // Si no tiene que subir imagenes solo graba datos:
    if (this.subir_imagenes) {
      ...
      // Si no tiene que subir imágenes:
      this.sendProfile(loading);
    }
  ...
  private sendProfile(loading) {
    ...
    if (this.subir_imagenes) this.user.guest.fastcheckin.imagenes = this.linksSubidos;
    else this.user.guest.fastcheckin.imagenes = this.huesped.fastcheckin.imagenes;
  ...
  vuelcaDatosFastcheckin() {
    ...
    this.fastcheckin.typeOfDocument = this.huesped.fastcheckin.typeOfDocument;
    this.fastcheckin.dni.identifier = this.huesped.fastcheckin.dni.identifier;
    this.fastcheckin.passport.identifier = this.huesped.fastcheckin.passport.identifier;
    this.fastcheckin.documento = this.user.guest.fastcheckin.documento;
    this.fastcheckin.date_exp = this.huesped.fastcheckin.date_exp;
    this.fastcheckin.name = this.huesped.fastcheckin.name;
    this.fastcheckin.surnameOne = this.huesped.fastcheckin.surnameOne;
    this.fastcheckin.surnameTwo = this.huesped.fastcheckin.surnameTwo;
    this.fastcheckin.birthday = this.huesped.fastcheckin.birthday;
  ...
  ContinuaFastcheckin() {
    ...
    this.pasosAnteriores.push(pasoAnterior);
    this.paso = 4;
    // Datos de Firma y Email:
    this.sigpad.fromDataURL(this.huesped.fastcheckin.signature, this.signaturePadOptions);
    this.email = this.huesped.fastcheckin.email;
    this.policysecurity = true;
  ...
  // En edción de un huésped, si se quiere consultar la información de las imágenes y editar la misma
  consultar_editar() {
    this.vuelcaDatosFastcheckin();
    this.erroresRegistroManual = new ErroresFormularioRegistro();
    this.paso = 0;
    this.subir_imagenes = false;
  }

  // En edición de un huésped, si se quiere escanear de nuevo las imágenes:
  escanear_nuevo() {
    this.paso = 1;
    this.subir_imagenes = true;
  }
